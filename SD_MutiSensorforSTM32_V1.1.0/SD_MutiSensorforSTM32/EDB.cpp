...
void EDB::edbWrite(unsigned long ee, const byte* p, unsigned int recsize)
{
  ...
  for (unsigned int i = 0; i < recsize; i++) // recsize是要用到多少地址去写入内容
  {
    _write_byte(ee++, *p++); // ee 是要写的地址，*p是要写入这个地址的内容
	// if (millis() > (num + 10000))
	// {
	// 	break;
	// }
...
{  
  ...
  for (unsigned i = 0; i < recsize; i++) //recsize是要用到多少地址去读取内容
  {
    *p++ = _read_byte(ee++);  //* p 是保存读取回来内容的变量。
	// if (millis() > (num + 10000))
	// {
	// 	break;
	// }
