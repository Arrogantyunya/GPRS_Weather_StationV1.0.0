...
char g_SD_Sensor_Data[256] = {0};

void Convert_SensorData_to_CSV(void)
{
  char AscllChar[64] = {0};
  unsigned char char_len = 0;
  unsigned char SensorData_Length = 0;

  memset(g_SD_Sensor_Data, 0x00, sizeof(g_SD_Sensor_Data));

  //写入日期
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.year / 1000 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.year % 1000 / 100 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.year % 100 / 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.year % 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = '/';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.month / 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.month % 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = '/';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.day / 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.day % 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = ' ';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.hour / 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.hour % 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = ':';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.minutes / 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = RtcTime.minutes % 10 + '0';
  g_SD_Sensor_Data[SensorData_Length++] = ',';

  /* ----------------写入室外温度数据---------------- */
  unsigned int Temperature_Temp = Muti_Sensor_Data.Air_Temp;
  //如果是无效值
  if ((Muti_Sensor_Data.Air_Temp >= 65535) && (Muti_Sensor_Data.Air_Temp_Flag != 1)){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';

  }else{
    //如果温度是负数
    if (Muti_Sensor_Data.Air_Temp_Flag == 1){
      Temperature_Temp = 65536 - Temperature_Temp;
      g_SD_Sensor_Data[SensorData_Length++] = '-';
    }

    Var_Convert.Var_Convert_CharArray(Temperature_Temp, AscllChar, &char_len);

    for (unsigned char i = 0; i < char_len; i++){
      if (char_len < 2){
        g_SD_Sensor_Data[SensorData_Length++] = '0';  //example: 数值是3，位数1位，将它变成文本：0.3
        g_SD_Sensor_Data[SensorData_Length++] = '.';
      }
       g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';

      if (char_len >= 2){
        if (char_len - i == 2)
          g_SD_Sensor_Data[SensorData_Length++] = '.';  //example: 数值是34，位数2位，将它变为文本 3.4，或者225，变为文本22.5
       }
    }
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入室外湿度数据---------------- */
  unsigned int Humidity_Temp = Muti_Sensor_Data.Air_Humi * 10;
  //如果是无效值
  if ((int)(Muti_Sensor_Data.Air_Humi) > 100){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
  }else{
    Var_Convert.Var_Convert_CharArray(Humidity_Temp, AscllChar, &char_len);
  ...
  /* ----------------写入室外光照数据---------------- */
  unsigned long Lux_Temp = Muti_Sensor_Data.Air_Lux;
  if (Muti_Sensor_Data.Air_Lux > 200000){
    ...
    Var_Convert.Var_Convert_CharArray(Lux_Temp, AscllChar, &char_len);
    for (unsigned char i = 0; i < char_len; i++)
      g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入室外大气压数据---------------- */
  unsigned long Atmos_Temp = Muti_Sensor_Data.Air_Atmos;
  //无效值
  if (Atmos_Temp >= 0xFFFFFFFF || Atmos_Temp == 0){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    ...
    Var_Convert.Var_Convert_CharArray(Atmos_Temp, AscllChar, &char_len);
    
    for (unsigned char i = 0; i < char_len; i++){
      if (char_len < 3){
        if (char_len - i == 2){
          g_SD_Sensor_Data[SensorData_Length++] = '0';  //example: 数值是23，位数2位，将它变成文本：0.23
          g_SD_Sensor_Data[SensorData_Length++] = '.';
        }
      }
      ...
      if (char_len >= 3){
        if (char_len - i == 3)
          g_SD_Sensor_Data[SensorData_Length++] = '.';  //example: 数值是80225，位数5位，将它变为文本 802.25kPa
        }
    }
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入室外紫外线数据---------------- */
  unsigned int UV_Temp = Muti_Sensor_Data.Air_UV;
  //无效值
  if (Muti_Sensor_Data.Air_UV >= 65535){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';    
    ...
    Var_Convert.Var_Convert_CharArray(UV_Temp, AscllChar, &char_len);
  ...
  /* ----------------写入室外二氧化碳数据---------------- */
  unsigned int CO2_Temp = Muti_Sensor_Data.Air_CO2;
  //无效值
  if (Muti_Sensor_Data.Air_CO2 >= 65535 || Muti_Sensor_Data.Air_CO2 == 0){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';      
  }else{
    Var_Convert.Var_Convert_CharArray(CO2_Temp, AscllChar, &char_len);
    for (unsigned char i = 0; i < char_len; i++)
      g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入室外TVOC数据---------------- */
  unsigned int TVOC_Temp = Muti_Sensor_Data.Air_TVOC;
  //无效值
  if (Muti_Sensor_Data.Air_TVOC >= 65535){
    ...
    Var_Convert.Var_Convert_CharArray(TVOC_Temp, AscllChar, &char_len);
    for (unsigned char i = 0; i < char_len; i++)
      g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入雨雪变送器开关量数据---------------- */
  unsigned char Rainfall_Temp = Muti_Sensor_Data.Rainfall;
  //无效值
  if (Muti_Sensor_Data.Rainfall == 0xFF){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';      
  }else{
    g_SD_Sensor_Data[SensorData_Length++] = Rainfall_Temp + '0';
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  /* ------------------------------------------------------- */

  /* ----------------写入室外风速数据---------------- */
  unsigned int Wind_Speed_Temp = Muti_Sensor_Data.Air_Wind_Speed * 10;
  //无效值
  if ((unsigned int)(Muti_Sensor_Data.Air_Wind_Speed) >= 0xFFFF){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';      
    ...
    Var_Convert.Var_Convert_CharArray(Wind_Speed_Temp, AscllChar, &char_len);
  ...
  /* ----------------写入室外风向数据---------------- */
  unsigned int Wind_DirCode_Temp = Muti_Sensor_Data.Wind_DirCode;
  //无效值
  if (Muti_Sensor_Data.Wind_DirCode >= 0xFFFF){
    g_SD_Sensor_Data[SensorData_Length++] = 'N';
    g_SD_Sensor_Data[SensorData_Length++] = 'U';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';
    g_SD_Sensor_Data[SensorData_Length++] = 'L';      
  }else{
    Var_Convert.Var_Convert_CharArray(Wind_DirCode_Temp, AscllChar, &char_len);
    for (unsigned char i = 0; i < char_len; i++)
      g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  }
  g_SD_Sensor_Data[SensorData_Length++] = ',';
  ...
  /* ----------------写入扩展ADC水位数据---------------- */
  unsigned int Water_Level_Temp = Muti_Sensor_Data.Water_Level;

  Var_Convert.Var_Convert_CharArray(Water_Level_Temp, AscllChar, &char_len);
  for (unsigned char i = 0; i < char_len; i++)
    g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  ...
  /* ----------------写入扩展ADC通道1水位数据---------------- */
  unsigned int ADC_Value1_Temp = Muti_Sensor_Data.ADC_Value1;

  Var_Convert.Var_Convert_CharArray(ADC_Value1_Temp, AscllChar, &char_len);
  for (unsigned char i = 0; i < char_len; i++)
    g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
  ...
  /* ----------------写入扩展ADC通道2水位数据---------------- */
  unsigned int ADC_Value2_Temp = Muti_Sensor_Data.ADC_Value2;

  Var_Convert.Var_Convert_CharArray(ADC_Value2_Temp, AscllChar, &char_len);
  for (unsigned char i = 0; i < char_len; i++)
    g_SD_Sensor_Data[SensorData_Length++] = AscllChar[i] + '0';
...
void Data_Acquisition(void)
{
  ...
  //读取大气二氧化碳和TVOC浓度(放在后面采集时因为该传感器需要通电预热)
  ...
  Convert_SensorData_to_CSV();

  if (Sys_Run_Para.g_RunMode == NWK_TF || Sys_Run_Para.g_RunMode == SMS_TF || Sys_Run_Para.g_RunMode == TF){
    if (RtcTime.year > 2000){
      SD_Ope.Create_DeviceID_Dir(SysHostID);
      SD_Ope.Create_Year_Dir(RtcTime.year);
      SD_Ope.Create_Month_Dir(RtcTime.month);
      SD_Ope.Create_SensorData_File();
      SD_Ope.Write_File_Headline();
      SD_Ope.Write_Sensor_Data_to_SD(g_SD_Sensor_Data);
    }
    else
      Serial.println("RTC date Error, SD card don't write data!");
  }
  ...
  Serial.print("Air uv: "); Serial.print(Muti_Sensor_Data.Air_UV); Serial.println("W/cm");
  ...
  #if (TYPE07 || TYPE09)
  Serial.print("Water_Level: "); Serial.println(Muti_Sensor_Data.Water_Level);
  Serial.print("Extend ADC channel1: "); Serial.println(Muti_Sensor_Data.ADC_Value1);
  Serial.print("Extend ADC channel2: "); Serial.println(Muti_Sensor_Data.ADC_Value2);
  #endif
